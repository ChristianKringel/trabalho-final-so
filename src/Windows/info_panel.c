...
// static void set_flight_color(WINDOW* win, Aviao* aviao) {
//     if (!win || !aviao) return;
//     int color_pair = (aviao->tipo == VOO_DOMESTICO) ? PAIR_DOM : PAIR_INTL;
//     wattron(win, COLOR_PAIR(color_pair));
// }

// static void set_flight_type_str(Aviao* aviao, char* buffer, size_t size) {
//     if (!aviao || !buffer || size == 0) return;
//     snprintf(buffer, size, "%c%02d", (aviao->tipo == VOO_DOMESTICO) ? 'D' : 'I', aviao->id);
// }
// static void reset_color(WINDOW* win) {
//     if (!win) return;
//     wattroff(win, A_BOLD);
//     wattroff(win, COLOR_PAIRS);
// }
