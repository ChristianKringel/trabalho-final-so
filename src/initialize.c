...
void inicializar_banqueiro(RecursosAeroporto* recursos, int pistas, int portoes, int torres) {
    for (int i = 0; i < MAX_AVIOES; i++) {
        for (int j = 0; j < N_RESOURCES; j++) {
            recursos->banco.alocacao[i][j] = 0;
            recursos->banco.necessidade[i][j] = 0;
        }
    }

    for (int j = 0; j < N_RESOURCES; j++) {
        recursos->banco.disponivel[j] = 0;
    }
    recursos->banco.disponivel[RECURSO_PISTA]  = pistas;
    recursos->banco.disponivel[RECURSO_PORTAO] = portoes;
    recursos->banco.disponivel[RECURSO_TORRE]  = 2;
...
void inicializar_recursos(RecursosAeroporto* recursos, int pistas, int portoes, int torres){
    ...
    inicializar_banqueiro(recursos, pistas, portoes, torres);
}
...
SimulacaoAeroporto* inicializar_simulacao(int pistas, int portoes, int torres, int tempo_simulacao, int max_avioes){
    ...
    pthread_mutex_init(&sim->recursos.mutex_operacao_inicio, NULL);
    create_log_file();
    return sim;
}